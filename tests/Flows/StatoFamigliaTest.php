 ...
 */
class StatoFamigliaTest extends AbstractAppTestCase
{
    /**
     * @var CPSUserProvider
     */
    ...
    public function testICanFillOutTheStatoFamigliaAsLoggedUser()
    {
        ...
        $fqcn = StatoFamiglia ::class;
        $flow = 'ocsdc.form.flow.statofamiglia';
        $servizio = $this->createServizioWithErogatore($erogatore, 'Stato famiglia', $fqcn, $flow);
        ...
        $currentPratica = $this->em->getRepository('AppBundle:StatoFamiglia')->find($currentPraticaId);
        $this->assertEquals(StatoFamiglia::class, get_class($currentPratica));
